...
from netsV2 import DGNLNet_fast
...
def main():
    ...
    # log_path = 'E:\Minor_Project\DGNL-Net\ckpt\DGNLNet_fast\ '+str(datetime.datetime.now()) + '.txt'
...
def train(net, optimizer):
    ...
    for epoch in range(epoch_num):
        ...
        loss = 0.0
        tqdm_bar = tqdm(train_loader, desc='Epoch {} loss: {:.4f} '.format(epoch+1, loss), unit='batch', bar_format='{desc}{percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]')
        ...
        for inputs, gts, dps in tqdm_bar:
            ...
            tqdm_bar.set_description('Epoch {} loss: {:.4f} '.format(epoch+1, loss))
