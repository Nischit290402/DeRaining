...
def make_dataset(root, is_train):
    if is_train:
        ...
        image = [(os.path.join(root, 'train', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'gtTrain', img_name.strip('\n'))) for img_name in
                 ground_t]
        depth = [(os.path.join(root, 'depthTrain', img_name.strip('\n'))) for img_name in
              depth_t]
        ...
        image = [(os.path.join(root, 'test', img_name.strip('\n'))) for img_name in
                 input]
        gt = [(os.path.join(root, 'gtTest', img_name.strip('\n'))) for img_name in
              ground_t]
        depth = [(os.path.join(root, 'depthTest', img_name.strip('\n'))) for img_name in
                 depth_t]
